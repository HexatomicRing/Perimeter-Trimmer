...
public class InventoryManager {
    ...
    public static boolean switchToTool(BlockState targetBlock) {
        MinecraftClient minecraftClient = MinecraftClient.getInstance();
        PlayerInventory playerInventory = minecraftClient.player.getInventory();
        int k = -1;
        for (int i = 0; i < playerInventory.main.size(); ++i) {
            if (getBlockBreakingSpeed(targetBlock, i) > 45f) {
                k = i;
            }
        }
        if (k != -1) {
            if (PlayerInventory.isValidHotbarIndex(k)) {
                playerInventory.selectedSlot = k;
            } else {
                minecraftClient.interactionManager.pickFromInventory(k);
            }
            minecraftClient.getNetworkHandler().sendPacket(new UpdateSelectedSlotC2SPacket(playerInventory.selectedSlot));
            return true;
        }
    ...
    public static String warningMessage() {
        ...
        if (!"survival".equals(minecraftClient.interactionManager.getCurrentGameMode().getName())) {
            return "perimeter_trimmer.fail.missing.survival";
        }

        if (InventoryManager.getInventoryItemCount(Blocks.PISTON) < 2) {
            return "perimeter_trimmer.fail.missing.piston";
        }

        if (InventoryManager.getInventoryItemCount(Blocks.REDSTONE_TORCH) < 1) {
            return "perimeter_trimmer.fail.missing.redstonetorch";
        }

        if (InventoryManager.getInventoryItemCount(Blocks.SLIME_BLOCK)<1){
            return "perimeter_trimmer.fail.missing.slime";
        }

        if (!InventoryManager.canInstantlyMinePiston()) {
            return "perimeter_trimmer.fail.missing.instantmine";
        }
        return null;
    }
